...
pub(crate) trait SealedChip {
    ...
    /// Release the buffer memory of every socket other than socket 0.
    ///
    /// MACRAW uses socket 0 alone, so it may as well have the chip's whole
    /// RX/TX memory — but on chips where the per-socket power-on defaults
    /// already add up to that memory, the other sockets must be set to zero
    /// first or the total is over-committed. Called right after socket 0's
    /// buffer sizes are written. The default does nothing, which is correct
    /// for chips whose `BUF_SIZE` fits alongside the defaults.
    async fn release_unused_socket_buffers<SPI: SpiDevice>(_spi: &mut SPI) -> Result<(), SPI::Error> {
        Ok(())
    }
}
