...
pub enum RegisterBlock {
    ...
    TxBuf = 0x02,
    RxBuf = 0x03,
    // Socket N's register block is `1 + 4N` (WIZnet `WIZCHIP_SREG_BLOCK(N)`).
    // Only their buffer-size registers are touched here, to hand their share
    // of the chip's memory to socket 0.
    Socket1 = 0x05,
    Socket2 = 0x09,
    Socket3 = 0x0D,
    Socket4 = 0x11,
...
impl super::SealedChip for W6300 {
    ...
    // 16 KB: the whole of the chip's RX (and TX) memory, and the largest value
    // a socket accepts. MACRAW uses socket 0 alone, and at the previous 4 KB a
    // 32-frame Art-Net burst lost 23 of its frames inside the chip. Requires
    // `release_unused_socket_buffers` below — see PATCHES.md.
    ...
    /// Zero `Sn_TX_BSR` (0x0200) and `Sn_RX_BSR` (0x0220) on sockets 1-7.
    ///
    /// The W6300 has 16 KB of RX and 16 KB of TX memory shared by eight
    /// sockets, the per-socket sizes may not sum past that, and every socket
    /// powers on at 2 KB - so the defaults already spend all of it. Socket 0
    /// can only be given the full 16 KB once the others hold none.
    async fn release_unused_socket_buffers<SPI: SpiDevice>(spi: &mut SPI) -> Result<(), SPI::Error> {
        const OTHERS: [RegisterBlock; 7] = [
            RegisterBlock::Socket1,
            RegisterBlock::Socket2,
            RegisterBlock::Socket3,
            RegisterBlock::Socket4,
            RegisterBlock::Socket5,
            RegisterBlock::Socket6,
            RegisterBlock::Socket7,
        ];
        for block in OTHERS {
            Self::bus_write(spi, (block, 0x0200), &[0]).await?; // Sn_TX_BSR
            Self::bus_write(spi, (block, 0x0220), &[0]).await?; // Sn_RX_BSR
        }
        Ok(())
    }
