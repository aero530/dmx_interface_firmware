...
impl<C: Chip, SPI: SpiDevice> WiznetDevice<C, SPI> {
    ...
    pub async fn new(spi: SPI, mac_addr: [u8; 6]) -> Result<Self, InitError<SPI::Error>> {
        ...
        // Sockets 1..n give their memory up *first*, so the total is never
        // over-committed even for an instant: on chips whose power-on defaults
        // already fill the shared memory, sizing socket 0 up before the others
        // are zeroed is undefined behaviour per the datasheet. No-op elsewhere.
        C::release_unused_socket_buffers(&mut this.spi).await?;
