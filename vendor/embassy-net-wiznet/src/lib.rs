// Vendored copy (see PATCHES.md). These are upstream style lints in code this
// repository does not otherwise touch; allowed here so `cargo clippy` in the
// firmware crates stays quiet without widening the diff against upstream.
#![allow(clippy::needless_borrow, clippy::identity_op, clippy::new_without_default)]
#![no_std]
