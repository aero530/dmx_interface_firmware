//! Standalone panel probe — a verbatim copy of the Rev 1 display path.
//!
...
//!
//! # Why this is a copy rather than something hand-rolled
//!
...
async fn main(spawner: Spawner) {
    ...
    info!("panel probe: Rev 1 display path, carrier board not involved");
    ...
    // Initial levels copied from `nucleo/src/main.rs`: CS, DC and RESET all
    // start HIGH.
    let cs = Output::new(p.PIN_10, Level::High);
    let dc = Output::new(p.PIN_12, Level::High);
    let reset = Output::new(p.PIN_13, Level::High);

    // The two differences that matter: CS belongs to ExclusiveDevice (so it
    // frames every transaction), and RES belongs to mipidsi.
    let spi_device = match ExclusiveDevice::new_no_delay(spi, cs) {
        Ok(d) => d,
        Err(_) => {
            error!("panel probe: could not build the SPI device");
            return;
        }
    };
    ...
    {
        Ok(d) => {
            info!("panel probe: mipidsi init returned Ok");
            d
        }
        Err(_) => {
            error!("panel probe: mipidsi init failed");
            return;
        }
    };
